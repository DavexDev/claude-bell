...
Two sound themes are bundled — pick whichever fits your taste:
...
- **`theme`** = which bundled sound pack to use for bare filenames — `"mac"` or `"retro"`. Ignored for absolute paths. Switch it with `claude-bell theme <name>`.
...
| Goal | How |
|------|-----|
| **Switch the whole vibe** (mac ↔ retro) | `claude-bell theme retro` (or edit `"theme"` in your config). |
...
| `theme [name]` | Show the current/available themes, or switch to one (`mac`, `retro`). |
...
(royalty-free, no third-party audio) — one set per theme in `sounds/mac/` and
`sounds/retro/`.
